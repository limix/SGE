...
#dirIndirCov_v2 allows to pass low rank 2x2 genetic matrix (DGE SGE their cov) - useful to test whether correlation between DGE and SGE is |1| or not.
from dirIndirCov_v2 import DirIndirCov
...
class DirIndirVD():
    ...
    #covs: missing values should be encoded as -999.  covs can be 1xN or Nx1 vector, or a NxK matrix (K number of covariates). intercept column not expected.
    ...
    def __init__(self, pheno = None, pheno_ID = None, covs = None, covs_ID = None, covariates_names = None, kinship_cm = None, kinship_cm_ID = None, cage_cm = None, cage_cm_ID = None, independent_covs = True, DGE = True, IGE = False, IEE = False, cageEffect = False, calc_ste = False, standardize_pheno = True, subset_IDs = None, vc_init = None, vc_init_type = 'lowrank', subsampling = False, SS_percent = 0.8, SS_repeat = 1000, covar_outfile_name = None, std_genos = True):
 
        self.parseNmore(pheno, pheno_ID, covs, covs_ID, covariates_names, kinship_cm, kinship_cm_ID, cage_cm, cage_cm_ID, independent_covs,standardize_pheno, subset_IDs, std_genos)
        ...
        if subsampling:
            self.sampleID_saved=copy.copy(self.sampleID)
            self.pheno_saved=copy.copy(self.pheno)
            self.covs_saved=copy.copy(self.covs)
            self.cage_saved=copy.copy(self.cage)
            self.kinship_saved=copy.copy(self.kinship)
            self.kinship_cm_saved=copy.copy(self.kinship_cm)
            self.kinship_cross_saved=copy.copy(self.kinship_cross)
            self.env_saved=copy.copy(self.env)
            self.env_cross_saved=copy.copy(self.env_cross)
            self.env_cm_saved=copy.copy(self.env_cm)
            self.uCage_saved = sp.unique(self.cage)

            output = []
            counter=0
            while ((len(output)<SS_repeat) and (counter<(SS_repeat*2))):
                counter += 1
                try:
                    res = self.run_SS(DGE = DGE, IGE = IGE, IEE = IEE, cageEffect = cageEffect, vc_init = vc_init, SS_percent = SS_percent)
                    output.append(res)
                    outfile_name = "".join([covar_outfile_name,str(len(output))])
                    toPick = self.getToPick()
                    pickle.dump(toPick,open(outfile_name,'wb'))

                except:
                    print "subsampling just failed - doing it again"
            self.output = output
        else:
            #VD defines the genetic, environmental and cage covariance matrices
            self.VD(DGE = DGE, IGE = IGE, IEE = IEE, cageEffect = cageEffect, vc_init = vc_init, vc_init_type = vc_init_type)
            #optimize estimates the variance components
            self.optimize(vc_init = vc_init)
    ...
    def parseNmore(self, pheno, pheno_ID, covs, covs_ID, covariates_names, kinship_cm, kinship_cm_ID, cage_cm, cage_cm_ID, independent_covs,standardize_pheno, subset_IDs,std_genos):
...
#        remove=uCage[0:1000]
#        idx_remove = sp.concatenate([sp.where(cage_cm==remove[i])[0] for i in range(len(remove))])
...
####hack to shorten runtime
        ...
        #3'. calculate real sample_size_cm
        is_real_cm = sp.array([cage_cm[i] in cage for i in range(cage_cm.shape[0])])
        self.real_sample_size_cm = is_real_cm.sum()
        #equivalent to:
        #idx_real_cm = sp.concatenate([sp.where(cage_cm==cage[i])[0] for i in range(len(cage))])
        #len(sp.unique(idx_real_cm))
        ...
        if independent_covs:
            ...
            if sp.any(~I):
                print 'Covariate cols '+str(sp.where(~I)[0])+' have been removed because linearly dependent on the others'
            covs = covs[:,I]
            if covariates_names is None:
                covariates_names = 'mean'
            else:
                covariates_names =  sp.append(['mean'],covariates_names)[I]
            print covariates_names

        # standardize covs except genotypes tagged by "direct" or "social"
        if std_genos:
            covs[:,1:] -= sp.delete(covs, 0, axis=1).mean(0)
            covs[:,1:] /= sp.delete(covs, 0, axis=1).std(0)
            ...
            is_not_geno = sp.logical_and(is_not_direct_geno,is_not_social_geno)
            idxs = sp.where(is_not_geno)[0]
        ...
        #    print covs[:,i].mean(0)
        #    print covs[:,i].std(0)
        # so mean is 1 sd 0 for mean then mean is ~0 and var 1 for all other non-genotype covariates    
    ...
    def VD(self, DGE, IGE, IEE, cageEffect,vc_init, vc_init_type):
...
#        check_outfile.close()
        ...
        if DGE and not IGE:
            ...
            if vc_init is not None:
                if vc_init_type == 'lowrank':
                    self._genoCov = DirIndirCov(self.kinship,Z,kinship_cm=self.kinship_cm,kinship_cross=self.kinship_cross,C=LowRankCov(2,1))
                elif vc_init_type == 'diagonal':
                    self._genoCov = DirIndirCov(self.kinship,Z,kinship_cm=self.kinship_cm,kinship_cross=self.kinship_cross,C=DiagonalCov(2))
            else:
    ...
    def get_VCs(self, calc_ste, DGE, IGE, IEE, cageEffect):
        ...
        if calc_ste:
            try:
                def get_rho_ste(va, vab, vb, V):
                    ...
                    Drho = sp.zeros(3)
                    Drho[0] = (-vab) / (2*va*sp.sqrt(va * vb))
                    Drho[1] = 1/sp.sqrt(va*vb) 
                    Drho[2] = (-vab) / (2*vb*sp.sqrt(va * vb))
                    rho_ste = sp.sqrt(sp.dot(Drho.T, sp.dot(V, Drho)))
                    return rho_ste
                ...
                if DGE and IGE:
                    ...
                    #this code only calculates STE when both DGE and SGE are in the model
        ...
        R['sample_size_cm'] = self.real_sample_size_cm
        
        #want to return a value for STE_Ad etc. even when no DGE etc. in model
        ...
        #correlation between DGE and SGE
        ...
        if DGE and (not IGE):
            ...
            R['total_gen_var'] = 1/covar_rescaling_factor(self._genoCov.K())
            R['corr_Ads'] = self._genoCov.C.K()[0,1]/sp.sqrt(self._genoCov.C.K()[0,0]*self._genoCov.C.K()[1,1])
        else:
            R['var_Ad'] = (-999)
            R['var_As'] = (-999)
            R['total_gen_var'] = (-999)
        ...
        if not IEE:
            ...
            R['corr_Eds'] = self._envCov.C.K()[0,1] / (sp.sqrt(R['var_Ed']) * sp.sqrt(R['var_Es']))
    ...
    def getToPick(self):
        return {'sampleID' : self.sampleID,'pheno' : self.pheno,'covs' : self.covs,'covar_mat' : self.getDirIndirVar().K()}

    #same as above
    def getToSave(self):
        return {'sampleID' : self.sampleID,'pheno' : self.pheno,'covs' : self.covs,'covar_mat' : self.getDirIndirVar().K()}
 
    def run_SS(self, DGE, IGE, IEE, cageEffect, vc_init, SS_percent):
    
        gc.collect()
        print "in subsampling"
        nb_cages_in= int(math.floor (SS_percent * len(self.uCage_saved)))
        cages_in = sp.random.choice(self.uCage_saved,size=nb_cages_in,replace=False)
        idxs_in = sp.concatenate([sp.where(self.cage_saved==cages_in[i])[0] for i in range(len(cages_in))])

        self.sampleID = self.sampleID_saved[idxs_in]
        self.pheno = self.pheno_saved[idxs_in,:]
        self.pheno -= self.pheno.mean(0)
        self.pheno /= self.pheno.std(0)
        self.covs = self.covs_saved[idxs_in,:]
        self.cage = self.cage_saved[idxs_in]
        self.kinship = self.kinship_saved[idxs_in,:][:,idxs_in]
        self.kinship_cm = copy.copy(self.kinship_cm_saved)
        self.kinship_cross = self.kinship_cross_saved[idxs_in,:]
        self.env = self.env_saved[idxs_in,:][:,idxs_in]
        self.env_cross = self.env_cross_saved[idxs_in,:]
        self.env_cm = copy.copy(self.env_cm_saved)
        print 'Shape self.sampleID ' + str(self.sampleID.shape)
        print 'Shape self.kinship_cm ' + str(self.kinship_cm.shape)

        # full model
        self.VD(DGE = DGE, IGE = IGE, IEE = IEE, cageEffect = cageEffect,vc_init = vc_init)
        self.optimize(vc_init = vc_init)
        output_SS = self.get_VCs (calc_ste = False, DGE = DGE, IGE = IGE, IEE = IEE, cageEffect = cageEffect)
        output = [len(idxs_in),output_SS['var_Ad'],output_SS['var_As'],output_SS['total_var'],output_SS['corr_Ads'],output_SS['conv']]
        return output
