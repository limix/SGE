...
motch = match(paste(all_QTLs[,'Name'],'_','chr',all_QTLs[,'Chr'],'_',all_QTLs[,'Pos (bp)'],'_','social',sep=''),paste(indiv_SGE_QTLs_betas[,1],indiv_SGE_QTLs_betas[,2],sep='_'))
any(is.na(motch))
#FALSE

all_QTLs$effect_size = round(indiv_SGE_QTLs_betas[motch,3],digits = 1)
...
any(is.na(motch))
#TRUE - osteoporosis - will need to fix manually
...
write.table(all_QTLs,file = 'plots_dir/SGE_QTLs.xls',sep='\t',quote = F, row.names = F, col.names = T)
