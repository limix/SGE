...
pvalues_file_dir = "".join(['output_dir/pvalues_perms_pointwise_conditional/DGE/',kinship_type,'_',subset,'_DGE_IGE_IEE_cageEffect'])
...
for k in range(nb_perms+1,101):
	...
	print(k)
	sp.random.shuffle(geno_matrix)
	pvalues = [-1] * nb_snps
	#first lmm run to initialize object
	lmm = qtl.test_lmm(snps=geno_matrix[:,0:1],pheno=saved['pheno'],covs = sp.concatenate([saved['covs'],social_geno_matrix[:,0:1]], axis=1),K=saved['covar_mat'],NumIntervalsDelta0=2,searchDelta=False)
